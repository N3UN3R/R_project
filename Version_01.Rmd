...
## Alle Bestandteile die als fehlerhaft gemeldet sind, in den Fahrzeugen die in unseren Städten zugelassen sind 
```{r, message=FALSE, warning = FALSE, cache = TRUE}

glimpse(fz_ges_filtered_years_bestandteile)

Bestandteile_Komponente_K1BE1 <-fread("Komponente/Komponente/Bestandteile_Komponente_K1BE1.csv", header = TRUE, data.table = FALSE)
glimpse(Bestandteile_Komponente_K1BE1)

#K1BE1_filtered <- inner_join(Bestandteile_Komponente_K1BE1, fz_ges_filtered_years_bestandteile, by = "ID_Karosserie")
...
# Daten einlesen und das richtige Trennzeichen angeben (hier wird ein Tabulator angenommen)
part_T07 <- fread("Einzelteil/Einzelteil/Einzelteil_T07.txt", sep = "\t", quote = "", header = FALSE, data.table = FALSE)

# Filtern und Daten anzeigen
part_T07 <- part_T07 %>% 
  filter((Fehlerhaft == 1) &
         (year(as.Date(Fehlerhaft_Datum)) >= 2014 & year(as.Date(Fehlerhaft_Datum)) <= 2016)) 

# Ausgabe der Struktur der gefilterten Daten
glimpse(part_T07)
...
```{r, message=FALSE, warning = FALSE, cache = TRUE}
# Datei einlesen
part_T07 <- read.table("Einzelteil/Einzelteil/Einzelteil_T07.txt", sep = "\t", header = TRUE, quote = "\"", stringsAsFactors = FALSE)
