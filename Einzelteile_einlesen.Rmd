...
#34
```{r, message=FALSE, warning = FALSE, cache = TRUE}
part_T34 <- read_file(paste0(filepath,"Data/Einzelteil/Einzelteil_T34.txt")) %>%
  str_replace_all(" \\| \\| ", ",") %>%
  str_replace_all("\"\"", "\"\n\"")
...
part_T35 <- read_file(paste0(filepath,"Data/Einzelteil/Einzelteil_T35.txt")) %>%
  str_replace_all("\\\\", ", ") %>%
  str_replace_all("(?<=\\S)(\"\\d+\")", "\n")
...
#36 !!!!!!!!!!!!!!!!!!!!!!!!!
```{r, message=FALSE, warning = FALSE, cache = TRUE}
part_T36 <- read_file(paste0(filepath,"Data/Einzelteil/Einzelteil_T36.txt")) %>%
  str_replace_all("  ", ",") %>%
  str_replace_all("\v", "\n")
glimpse(part_T36)
part_T35 <- fread(part_T36, sep = ",")
glimpse(part_T36)
...
txt <- gsub("\t","\n",txt, fixed = TRUE)
glimpse(txt)
part_T36 <- fread(txt, sep = ",", data.table = FALSE, drop = c(1,2)) %>% 
 # filter(Fehlerhaft == 1 &
      #   ((year(as.Date(Fehlerhaft_Datum)) >= 2014 & year(as.Date(Fehlerhaft_Datum)) <= 2016)))
glimpse(part_T36)
```
...
#39 
```{r, message=FALSE, warning = FALSE, cache = TRUE}
part_T39 <- read_file(paste0(filepath,"Data/Einzelteil/Einzelteil_T39.txt")) %>%
  str_replace_all("\\\\", ",") %>%
  str_replace_all("\a", "\n") 
