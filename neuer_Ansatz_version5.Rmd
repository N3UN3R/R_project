...
```
```{r, message=FALSE, warning = FALSE, cache = TRUE}
test_klein <- select(part_T01,ID_T01,Fehlerhaft_Datum_T01, Fehlerhaft_Fahrleistung_T01)
...
```{r, message=FALSE, warning = FALSE, cache = TRUE}
test_all_df <- select(all_df_data,ID_Fahrzeug, ID_T01)
...
print(sum(is.na(test_all_df$ID_T01)))


bereinigter_dataframe <- na.omit(test_all_df)


glimpse(bereinigter_dataframe)

#print(sum(any(is.na(test_all_df$ID_T01))))
```

```{r, message=FALSE, warning = FALSE, cache = TRUE}
#test_ner <- left_join(all_df_data, select(part_T01, -Fehlerhaft,-Werksnummer,-Herstellernummer,-Produktionsdatum), by = "ID_T01")
...
#test_klein <- select(part_T01,ID_T01,Fehlerhaft_Datum, Fehlerhaft_Fahrleistung)

result_df <- inner_join(bereinigter_dataframe, test_klein, by = "ID_T01")



#df_neu <- inner_join(all_df_data, part_T01, by = "ID_T01") %>%
 # select(ID_Fahrzeug, ID_T01, Fehlerhaft_Datum, Fehlerhaft_Fahrleistung)
...
```{r, message=FALSE, warning = FALSE, cache = TRUE}
test_all_df2 <- select(all_df_data,ID_Fahrzeug, ID_T02)

test_klein2 <- select(part_T02,ID_T02,Fehlerhaft_Datum_T02, Fehlerhaft_Fahrleistung_T02)

bereinigter_dataframe2 <- na.omit(test_all_df2)


glimpse(bereinigter_dataframe2)

result_df2 <- inner_join(bereinigter_dataframe2, test_klein2, by = "ID_T02")
...
result_df_final <- inner_join(result_df, result_df2, by = "ID_Fahrzeug")


glimpse(result_df_final)
#print(sum(any(is.na(test_all_df$ID_T01))))
```
...
```{r, message=FALSE, warning = FALSE, cache = TRUE}
print("starts")
write.csv(result_df_final, "all_df_data.csv", row.names = FALSE)
