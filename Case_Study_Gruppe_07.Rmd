...
```{r, message=FALSE, warning = FALSE, cache = TRUE}
# Einlesen der Daten
zulassungen <- fread("Data/Zulassungen/Zulassungen_alle_Fahrzeuge.csv", header = TRUE, data.table = FALSE, encoding = "Latin-1" )
...
bestandteile_motor_K1BE1 <-fread("Data/Komponente/Bestandteile_Komponente_K1BE1.csv", header = TRUE, data.table = FALSE)

# Einlesen der Komponente
bestandteile_motor_K1BE2 <-fread("Data/Komponente/Bestandteile_Komponente_K1BE2.csv", header = TRUE, data.table = FALSE)

# Einlesen der Komponente
bestandteile_motor_K1DI1 <-fread("Data/Komponente/Bestandteile_Komponente_K1DI1.csv", header = TRUE, data.table = FALSE)

# Einlesen der Komponente
bestandteile_motor_K1DI2 <-fread("Data/Komponente/Bestandteile_Komponente_K1DI2.csv", header = TRUE, data.table = FALSE)

# Einlesen der Komponente
bestandteile_sitze_K2LE1 <-fread("Data/Komponente/Bestandteile_Komponente_K2LE1.csv", header = TRUE, data.table = FALSE)

# Einlesen der Komponente
bestandteile_sitze_K2LE2 <-fread("Data/Komponente/Bestandteile_Komponente_K2LE2.csv", header = TRUE, data.table = FALSE)

# Einlesen der Komponente
bestandteile_sitze_K2ST1 <-fread("Data/Komponente/Bestandteile_Komponente_K2ST1.csv", header = TRUE, data.table = FALSE)

# Einlesen der Komponente
bestandteile_sitze_K2ST2 <-fread("Data/Komponente/Bestandteile_Komponente_K2ST2.csv", header = TRUE, data.table = FALSE)

# Einlesen der Komponente
bestandteile_schaltung_K3AG1 <-fread("Data/Komponente//Bestandteile_Komponente_K3AG1.csv", header = TRUE, data.table = FALSE)

# Einlesen der Komponente
bestandteile_schaltung_K3AG2 <-fread("Data/Komponente/Bestandteile_Komponente_K3AG2.csv", header = TRUE, data.table = FALSE)

# Einlesen der Komponente
bestandteile_schaltung_K3SG1 <-fread("Data/Komponente/Bestandteile_Komponente_K3SG1.csv", header = TRUE, data.table = FALSE)

# Einlesen der Komponente
bestandteile_schaltung_K3SG2 <-fread("Data/Komponente/Bestandteile_Komponente_K3SG2.csv", header = TRUE, data.table = FALSE)

# Einlesen der Komponente
bestandteile_carosserie_K4 <-fread("Data/Komponente/Bestandteile_Komponente_K4.csv", header = TRUE, data.table = FALSE)

# Einlesen der Komponente
bestandteile_carosserie_K5 <-fread("Data/Komponente/Bestandteile_Komponente_K5.csv", header = TRUE, data.table = FALSE)

# Einlesen der Komponente
bestandteile_carosserie_K6 <-fread("Data/Komponente/Bestandteile_Komponente_K6.csv", header = TRUE, data.table = FALSE)

# Einlesen der Komponente
bestandteile_carosserie_K7 <-fread("Data/Komponente/Bestandteile_Komponente_K7.csv", header = TRUE, data.table = FALSE)
```
...
### Einzelteil T02 einlesen
```{r, message=FALSE, warning = FALSE, cache = TRUE}

path = "Data/Einzelteil/Einzelteil_T02.txt"
...
```{r, message=FALSE, warning = FALSE, cache = TRUE}
part_T05 <- fread("Data/Einzelteil/Einzelteil_T05.csv", header = TRUE, data.table = FALSE, drop= c(1,2)) %>% 
 filter((Fehlerhaft.x == 1 | Fehlerhaft.y == 1) &
         ((year(as.Date(Fehlerhaft_Datum.x)) >= 2014 & year(as.Date(Fehlerhaft_Datum.x)) <= 2016) |
          (year(as.Date(Fehlerhaft_Datum.y)) >= 2014 & year(as.Date(Fehlerhaft_Datum.y)) <= 2016)))
...
### Einzelteil T07 einlesen
```{r, message=FALSE, warning = FALSE, cache = TRUE}
txt <- readLines("Data/Einzelteil/Einzelteil_T07.txt")
txt <- gsub("\"\t\"",",",txt)
txt <- gsub("\"\t",",",txt)
txt <- gsub("\t\"",",",txt)
txt <- gsub("\t",",",txt)
...
### Einzelteil T08 einlesen
```{r, message=FALSE, warning = FALSE, cache = TRUE}
part_T08 <- fread("Data/Einzelteil/Einzelteil_T08.csv", header = TRUE, data.table = FALSE) %>% 
  filter(Fehlerhaft == 1 &
         ((year(as.Date(Fehlerhaft_Datum)) >= 2014 & year(as.Date(Fehlerhaft_Datum)) <= 2016)))
#glimpse(part_T08)
```
...
### Einzelteil T09 einlesen
```{r, message=FALSE, warning = FALSE, cache = TRUE}

part_T09 <- read_file("Data/Einzelteil/Einzelteil_T09.txt") %>% 
  str_replace_all('\\\"', '') %>%
  str_replace_all('\\\\', ',') %>%
  str_replace_all('\v','\n')
...
### Einzelteil T10 einlesen
```{r, message=FALSE, warning = FALSE, cache = TRUE}
part_T10 <- fread("Data/Einzelteil/Einzelteil_T10.csv", header = TRUE, data.table = FALSE) %>% 
  filter(Fehlerhaft == 1 &
         ((year(as.Date(Fehlerhaft_Datum)) >= 2014 & year(as.Date(Fehlerhaft_Datum)) <= 2016)))
#glimpse(part_T10)
```
...
### Einzelteil T12 einlesen
```{r, message=FALSE, warning = FALSE, cache = TRUE}
part_T12 <- fread("Data/Einzelteil/Einzelteil_T12.csv", header = TRUE, data.table = FALSE, drop = c(1,2)) %>% 
    filter((Fehlerhaft.x == 1 | Fehlerhaft.y == 1 | Fehlerhaft == 1 ) &
         ((year(as.Date(Fehlerhaft_Datum)) >= 2014 & year(as.Date(Fehlerhaft_Datum)) <= 2016) |
          (year(as.Date(Fehlerhaft_Datum.x)) >= 2014 & year(as.Date(Fehlerhaft_Datum.x)) <= 2016) |
          (year(as.Date(Fehlerhaft_Datum.y)) >= 2014 & year(as.Date(Fehlerhaft_Datum.y)) <= 2016)))
...
### Einzelteil T15 einlesen
```{r, message=FALSE, warning = FALSE, cache = TRUE}
part_T15 <- fread("Data/Einzelteil/Einzelteil_T15.csv", header = TRUE, data.table = FALSE, drop= c(1,2)) %>% 
 filter((Fehlerhaft.x == 1 | Fehlerhaft.y == 1) &
         ((year(as.Date(Fehlerhaft_Datum.x)) >= 2014 & year(as.Date(Fehlerhaft_Datum.x)) <= 2016) |
          (year(as.Date(Fehlerhaft_Datum.y)) >= 2014 & year(as.Date(Fehlerhaft_Datum.y)) <= 2016)))
...
### Einzelteil T16 einlesen
```{r, message=FALSE, warning = FALSE, cache = TRUE}
part_T16 <- read_file("Data/Einzelteil/Einzelteil_T16.txt") %>%
  str_replace_all(" \\| \\| ", ",") %>%
  str_replace_all("\t","\n")
...
### Einzelteil T17 einlesen
```{r, message=FALSE, warning = FALSE, cache = TRUE}
part_T17 <- fread("Data/Einzelteil/Einzelteil_T17.csv", header = TRUE, data.table = FALSE, drop = c(1,2)) %>% 
  filter((Fehlerhaft.x == 1 | Fehlerhaft.y == 1 | Fehlerhaft == 1 ) &
         ((year(as.Date(Fehlerhaft_Datum)) >= 2014 & year(as.Date(Fehlerhaft_Datum)) <= 2016) |
          (year(as.Date(Fehlerhaft_Datum.x)) >= 2014 & year(as.Date(Fehlerhaft_Datum.x)) <= 2016) |
          (year(as.Date(Fehlerhaft_Datum.y)) >= 2014 & year(as.Date(Fehlerhaft_Datum.y)) <= 2016)))
...
### Einzelteil T18 einlesen
```{r, message=FALSE, warning = FALSE, cache = TRUE}
part_T18 <- fread("Data/Einzelteil/Einzelteil_T18.csv", header = TRUE, data.table = FALSE) %>% 
  filter(Fehlerhaft == 1 &
         ((year(as.Date(Fehlerhaft_Datum)) >= 2014 & year(as.Date(Fehlerhaft_Datum)) <= 2016)))
#glimpse(part_T18)
```
...
### Einzelteil T19 einlesen
```{r, message=FALSE, warning = FALSE, cache = TRUE}
part_T19 <- fread("Data/Einzelteil/Einzelteil_T19.csv", header = TRUE, data.table = FALSE) %>% 
  filter(Fehlerhaft == 1 &
         ((year(as.Date(Fehlerhaft_Datum)) >= 2014 & year(as.Date(Fehlerhaft_Datum)) <= 2016)))
#glimpse(part_T19)
```
...
### Einzelteil T20 einlesen
```{r, message=FALSE, warning = FALSE, cache = TRUE}
#txt <- readLines(paste0(filepath,"Einzelteil/Einzelteil/Einzelteil_T20.txt"))
txt <- readLines("Data/Einzelteil/Einzelteil_T20.txt")
txt <- gsub(" | | ",",",txt, fixed = TRUE)
txt <- gsub(" ","\n",txt, fixed = TRUE)
...
### Einzelteil T21 einlesen
```{r, message=FALSE, warning = FALSE, cache = TRUE}
part_T21 <- fread("Data/Einzelteil/Einzelteil_T21.csv", header = TRUE, data.table = FALSE,drop = c(1,2,10)) %>% 
  filter(Fehlerhaft == 1 &
         ((year(as.Date(Fehlerhaft_Datum)) >= 2014 & year(as.Date(Fehlerhaft_Datum)) <= 2016)))
#glimpse(part_T21)
```
...
### Einzelteil T23 einlesen
```{r, message=FALSE, warning = FALSE, cache = TRUE}
part_T23 <- fread("Data/Einzelteil/Einzelteil_T23.csv", header = TRUE, data.table = FALSE,drop = c(1,2)) %>% 
 filter((Fehlerhaft.x == 1 | Fehlerhaft.y == 1) &
         ((year(as.Date(Fehlerhaft_Datum.x)) >= 2014 & year(as.Date(Fehlerhaft_Datum.x)) <= 2016) |
          (year(as.Date(Fehlerhaft_Datum.y)) >= 2014 & year(as.Date(Fehlerhaft_Datum.y)) <= 2016))) 
...
### Einzelteil T26 einlesen
```{r, message=FALSE, warning = FALSE, cache = TRUE}
part_T26 <- fread("Data/Einzelteil/Einzelteil_T26.csv", header = TRUE, data.table = FALSE) %>% 
  filter(Fehlerhaft == 1 &
         ((year(as.Date(Fehlerhaft_Datum)) >= 2014 & year(as.Date(Fehlerhaft_Datum)) <= 2016)))
#glimpse(part_T26)
```
...
### Einzelteil T27 einlesen
```{r, message=FALSE, warning = FALSE, cache = TRUE}
part_T27 <- read_file("Data/Einzelteil/Einzelteil_T27.txt")%>%
  str_replace_all(" \\| \\| ", ",") %>%
  str_replace_all("\a", "\n") 
...
### Einzelteil T32 einlesen
```{r, message=FALSE, warning = FALSE, cache = TRUE}
part_T32 <- fread("Data/Einzelteil/Einzelteil_T32.csv", header = TRUE, data.table = FALSE, drop = c(1.2)) %>% 
 filter((Fehlerhaft.x == 1 | Fehlerhaft.y == 1) &
         ((year(as.Date(Fehlerhaft_Datum.x)) >= 2014 & year(as.Date(Fehlerhaft_Datum.x)) <= 2016) |
          (year(as.Date(Fehlerhaft_Datum.y)) >= 2014 & year(as.Date(Fehlerhaft_Datum.y)) <= 2016))) 
...
### Einzelteil T34 einlesen
```{r, message=FALSE, warning = FALSE, cache = TRUE}
part_T34 <- read_file("Data/Einzelteil/Einzelteil_T34.txt") %>%
  str_replace_all(" \\| \\| ", ",") %>%
  str_replace_all("\"\"", "\"\n\"")
...
### Einzelteil T35 einlesen
```{r, message=FALSE, warning = FALSE, cache = TRUE}
part_T35 <- read_file("Data/Einzelteil/Einzelteil_T35.txt") %>%
  str_replace_all("\\\\", ", ") %>%
  str_replace_all("(?<=\\S)(\"\\d+\")", "\n")
...
### Einzelteil T37 einlesen
```{r, message=FALSE, warning = FALSE, cache = TRUE}
part_T37 <- fread("Data/Einzelteil/Einzelteil_T37.csv", header = TRUE, data.table = FALSE, drop = c(1,2,10)) %>% 
  filter(Fehlerhaft == 1 &
         ((year(as.Date(Fehlerhaft_Datum)) >= 2014 & year(as.Date(Fehlerhaft_Datum)) <= 2016)))
#glimpse(part_T37)
```
...
### Einzelteil T38 einlesen
```{r, message=FALSE, warning = FALSE, cache = TRUE}
part_T38 <- fread("Data/Einzelteil/Einzelteil_T38.csv", header = TRUE, data.table = FALSE, drop = c(1,2)) %>% 
 filter((Fehlerhaft.x == 1 | Fehlerhaft.y == 1) &
         ((year(as.Date(Fehlerhaft_Datum.x)) >= 2014 & year(as.Date(Fehlerhaft_Datum.x)) <= 2016) |
          (year(as.Date(Fehlerhaft_Datum.y)) >= 2014 & year(as.Date(Fehlerhaft_Datum.y)) <= 2016))) 
...
### Einzelteil T39 einlesen
```{r message=FALSE, warning=FALSE, cache=FALSE}
part_T39 <- read_file("Data/Einzelteil/Einzelteil_T39.txt") %>%
  str_replace_all("\\\\", ",") %>%
  str_replace_all("\a", "\n") 
...
### Einzelteil T40 einlesen
```{r, message=FALSE, warning = FALSE, cache = TRUE}
part_T40 <- fread("Data/Einzelteil/Einzelteil_T40.csv", header = TRUE, data.table = FALSE, drop = c(1,2,10)) %>% 
  filter(Fehlerhaft == 1 &
         ((year(as.Date(Fehlerhaft_Datum)) >= 2014 & year(as.Date(Fehlerhaft_Datum)) <= 2016)))
...
Im Server sind die Ausgaben und Ergebnisse, die in den Anzeigen der App zu sehen sind, erstellt worden. Dabei wurden sogenannte reaktive Funktionen genutzt. Diese ändern die Ausgabe ihres Ergebnisses und reagieren auf die Eingabe des Benutzers.
...
In der Karte selbst sind dabei die acht Gemeinden der Werkstätten mit blauen Markern hervorgehoben. An diesen Markern ist ein Pop-up integriert, welche die Prognosen für 2017 der jeweiligen Gemeinde anzeigt. Dabei bezieht sich diese Prognose, wie beschrieben nur auf das ausgwählte Einzelteil.
